...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] text-slate-900 selection:bg-blue-100 selection:text-blue-700 font-sans overflow-x-hidden relative">
      ...
      {/* --- 사이드 메뉴 오버레이 (absolute) --- */}
      <div className={`absolute inset-0 z-50 transition-all duration-500 ${isMenuOpen ? 'visible' : 'invisible'}`}>
        <div 
          className={`absolute inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity duration-500 ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`}
          ...
          <ul className="space-y-6">
            <li>
              <button 
                ...
                className="flex items-center gap-4 text-lg font-bold text-slate-600 hover:text-blue-600 transition-all group w-full text-left outline-none"
              >
                <span className="w-11 h-11 flex items-center justify-center rounded-2xl bg-slate-50 text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 text-xl shrink-0">
                  <span className="relative -top-[0.5px]">🔍</span>
                </span>
                <span className="leading-none mt-0.5">반주 검색 하기</span>
              </button>
            </li>
            ...
            <li>
              {/* 즐겨찾기 목록 안내 메시지 추가 */}
              <button 
                onClick={() => alert("즐겨찾기 목록 기능은 아직 준비 중입니다.")}
                className="flex items-center gap-4 text-lg font-bold text-slate-600 hover:text-blue-600 transition-all group w-full text-left outline-none"
              >
                <span className="w-11 h-11 flex items-center justify-center rounded-2xl bg-blue-50 text-blue-500 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 text-xl shrink-0">
                  <span className="relative -top-[0.5px]">⭐</span>
                </span>
                <span className="leading-none mt-0.5">즐겨찾기 목록</span>
              </button>
            </li>
            <li>
              {/* 신곡 알림 설정 안내 메시지 추가 */}
              <button 
                onClick={() => alert("신곡 알림 설정 기능은 아직 준비 중입니다.")}
                className="flex items-center gap-4 text-lg font-bold text-slate-600 hover:text-blue-600 transition-all group w-full text-left outline-none"
              >
                <span className="w-11 h-11 flex items-center justify-center rounded-2xl bg-indigo-50 text-indigo-500 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 text-xl shrink-0">
                  <span className="relative -top-[0.5px]">🔔</span>
                </span>
                <span className="leading-none mt-0.5">신곡 알림 설정</span>
              </button>
            </li>
            <li className="pt-6 border-t border-slate-100">
              ...
              <button className="flex items-center gap-4 text-lg font-bold text-slate-400 hover:text-slate-600 transition-all w-full text-left group outline-none">
                <span className="w-11 h-11 flex items-center justify-center rounded-2xl bg-slate-50 text-slate-400 group-hover:bg-slate-200 transition-all text-xl shrink-0">
                  <span className="relative -top-[0.5px]">👤</span>
                </span>
                <span className="leading-none mt-0.5">사용자 설정</span>
              </button>
            </li>
          </ul>
